...
func (bin unixRecycleBin) Recycle(filepath string) error {
	...
	// }
	//add by zhangzhenjiu ------------------------------------------------------------
	if err := fs.MkdirAll(bin.Path+"/files", os.ModePerm); err != nil {
		return err
	}
	if err := fs.MkdirAll(bin.Path+"/info", os.ModePerm); err != nil {
		return err
	}
	//-------------------------------------------------------------------------
	...
	//deletionDate := time.Now().Format("2006-01-02T15:04:05") // edit by zhangzhenjiu
	deletionDate := fmt.Sprintf("%d", time.Now().UnixNano())
...
func (bin unixRecycleBin) List() error {
	return nil
}
